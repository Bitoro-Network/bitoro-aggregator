...
import "../interfaces/IBitoroOrderBook.sol";
...
contract ProxyFactory is Storage, ProxyBeacon, DebtManager, ProxyConfig, OwnableUpgradeable {
    ...
    struct BitoroOrderParams {
        ...
        uint32 deadline; // 1e0
        bytes32 referralCode;
        IBitoroOrderBook.PositionOrderExtra extra;
    }
    ...
    event BitoroCall(address target, uint256 value, bytes data);

    function initialize(address weth_, address liquidityPool) external initializer {
        __Ownable_init();  // safeOwnerableUpgradeable from bitoro-protocol
        _weth = weth_;
    ...
    function isKeeper(address keeper) public view returns (bool) {
        return _keepers[keeper];  // storage.sol
    }
    ...
    function setBitoroOrderBook(address bitoroOrderBook) external onlyOwner {
        _bitoroOrderBook = bitoroOrderBook;
    }
    ...
    function openPositionV3(
        ...
        BitoroOrderParams calldata bitoroParams,
        uint256 bitoroValue
    ) external payable {
        require(msg.value >= bitoroValue, "InsufficientValue");
        _openPosition(args, msg.value - bitoroValue);
        if (bitoroParams.subAccountId != bytes32(0)) {
            require(msg.sender == _getSubAccountOwner(bitoroParams.subAccountId), "SubAccountNotMatch");
            IBitoroOrderBook(_bitoroOrderBook).placePositionOrder3{ value: bitoroValue }(
                bitoroParams.subAccountId,
                bitoroParams.collateralAmount,
                bitoroParams.size,
                bitoroParams.price,
                bitoroParams.profitTokenId,
                bitoroParams.flags,
                bitoroParams.deadline,
                bitoroParams.referralCode,
                bitoroParams.extra
            );
        }
    }
