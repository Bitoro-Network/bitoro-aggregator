...
describe("Baseline", () => {
  ...
  async function deployTokenFixture() {
    ...
    const [priceUpdater, ] = await ethers.getSigners()
    // fork begins
  ...
  it("baseline", async () => {
    ...
    /**
     * Set config values for asset token in projectId protocol
     * @param1 uint256, projectId, here projectId = 1
     * @param2 address, asset token, here assetToken is usdc
     * @param3 uint256[], config values
     */
    ...
    /**
     * Create proxy contract for trader1
     * @param1 projectId, here PROJECT_GMX=1
     * @param2 address, collateral token
     * @param3 address, asset token
     * @param4 boolean, isLong
     */
    ...
    console.log('getPendingOrderkeys = ', await proxy.getPendingGmxOrderKeys())
    var orderKey = ethers.utils.solidityKeccak256(["address", "uint256"], [proxy.address, 1])

    await setGmxPrice("1296.5")
    console.log('before increasePosition')
  ...
  it("baseline2", async () => {
    ...
    console.log("factory 2 = ", factory.address)
    console.log("aggregator 2 = ", aggregator.address)
    // console.log("proxy = ", _proxy);

    await factory.connect(trader1).createProxy(PROJECT_GMX, weth.address, weth.address, true)
    const [_proxy] = await factory.getProxiesOf(trader1.address)
    console.log('_proxy 2 = ', _proxy)

    // const proxy = await ethers.getContractAt("GmxAdapter", _proxy)
    const executionFee = await gmxPositionRouter.minExecutionFee()
    console.log('executionFee 2 = ', executionFee)
