...
describe("GmxOrder", () => {
  ...
  it("cancel timeout orders 2", async () => {
    ...
    var copyOrders = pendingOrders.concat("0x100000000000000001000000000052e58b04c2555600000000000000634fa2a4")
    await proxy.cancelTimeoutOrders(copyOrders)
    var pendingOrders = await proxy.getPendingGmxOrderKeys()
    expect(pendingOrders.length).to.equal(0)
    console.log(await pendingOrders)
  })
  ...
  it("tpsl order", async () => {
    ...
    const executionFee2 = await gmxOrderBook.minExecutionFee()
    console.log(executionFee2)
    ...
    var pendingOrders = await proxy.getPendingGmxOrderKeys()
    expect(pendingOrders.length).to.equal(3)

    var tpslOrders = await proxy.getTpslOrderKeys(pendingOrders[0])
    expect(tpslOrders[0]).to.equal(pendingOrders[1])
    expect(tpslOrders[1]).to.equal(pendingOrders[2])

    tpslOrders = await proxy.getTpslOrderKeys(pendingOrders[1])
    expect(tpslOrders[0]).to.equal(zeroBytes32)
    expect(tpslOrders[1]).to.equal(zeroBytes32)

    tpslOrders = await proxy.getTpslOrderKeys(pendingOrders[2])
    expect(tpslOrders[0]).to.equal(zeroBytes32)
    expect(tpslOrders[1]).to.equal(zeroBytes32)

    await proxy.connect(trader1).cancelOrders([pendingOrders[0]])
    var pendingOrders = await proxy.getPendingGmxOrderKeys()
    expect(pendingOrders.length).to.equal(0)
  ...
  it("tpsl order 3", async () => {
    ...
    var tpslOrders = await proxy.getTpslOrderKeys(pendingOrders[0])
    expect(tpslOrders[0]).to.equal(zeroBytes32)
    expect(tpslOrders[1]).to.equal(pendingOrders[1])
  ...
  it("cancel", async () => {
    ...
    await proxy.connect(trader1).cancelOrders([pendingOrders[1]])
    var pendingOrders = await proxy.getPendingGmxOrderKeys()
    expect(pendingOrders.length).to.equal(2)

    await time.increase(180)
    await proxy.connect(trader1).cancelOrders(pendingOrders)
    var pendingOrders = await proxy.getPendingGmxOrderKeys()
    expect(pendingOrders.length).to.equal(0)
  })
  ...
  it("trade", async () => {
    ...
    await factory.connect(trader1).openPositionV2(
      {
        ...
        minOut: toWei("0.011117352"),
        borrow: toWei("0.023333333333333334"),
        sizeUsd: toWei("1292.5"),
        priceUsd: toWei("1292.5"),
        tpPriceUsd: toWei("1392.5"),
        slPriceUsd: toWei("1192.5"),
        flags: 0x40 | 0x08,
        referralCode: zeroBytes32,
      },
      { value: executionFee.mul(3).add(3) }
    ...
    var pendingOrders0 = await proxy.getPendingGmxOrderKeys()
    expect(pendingOrders0.length).to.equal(6)
    ...
    await proxy.connect(trader1).withdraw()

    var pendingOrders1 = await proxy.getPendingGmxOrderKeys()
    expect(pendingOrders1.length).to.equal(5)
    ...
    var pendingOrders2 = await proxy.getPendingGmxOrderKeys()
    expect(pendingOrders2.length).to.equal(3)
  ...
  it("trade 2", async () => {
    ...
    var pendingOrders0 = await proxy.getPendingGmxOrderKeys()
    expect(pendingOrders0.length).to.equal(3)
    console.log(pendingOrders0)
    ...
    var pendingOrders1 = await proxy.getPendingGmxOrderKeys()
    expect(pendingOrders1.length).to.equal(2)
    expect(pendingOrders1.concat().sort()).to.deep.equal(pendingOrders0.slice(1).sort())
    console.log(pendingOrders1)
    ...
    await factory.connect(trader1).closePositionV2(
      ...
      { value: executionFee.mul(2).add(2) }
    )

    var pendingOrders2 = await proxy.getPendingGmxOrderKeys()
    expect(pendingOrders2.length).to.equal(4)
